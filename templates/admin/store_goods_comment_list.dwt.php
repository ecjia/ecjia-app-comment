...
<div class="store-goods_comment-storeinfo">
	<div class="storeinfo">
	    ...
	    <div class="comment-rank-info">
	    	...
	    	<p class="common-p">
                {t domain="comment"}综合评分：{/t}{section name=loop loop=$comment.comment_rank}<i class="fontello-icon-star" style="color:#FF9933;"></i>{/section}
	    			   {section name=loop loop=5-$comment.comment_rank}<i class="fontello-icon-star" style="color:#bbb;"></i>{/section}
	    	</p>
	    	<p class="common-p">{t domain="comment"}全部评论：{/t}<a style="text-decoration:none;" href="javascript:;">{$total_count}</a></p>
	    	<p class="common-p rank-rate">{t domain="comment"}好评率：{/t}{if $comment_percent}{$comment_percent}%{/if}</p>
	    </div>
	</div>
</div>
{/if}
<div class="nav-heading">
	<ul class="nav-status">
		...
		<li class="hide-rank"><a class="data-pjax btn no-show-rank" href='{url path="comment/admin/store_goods_comment_list" args="store_id={$store_id}{if $smarty.get.status neq null}&status={$smarty.get.status}{/if}{if $smarty.get.has_img neq null}&has_img={$smarty.get.has_img}{/if}{if $select_status}&select_status={$select_status}{/if}{if $select_img}&select_img={$select_img}{/if}"}' style="padding:3px 5px;">
		...
		<table class="table table-striped smpl_tbl table-hide-edit">
			<thead>
				<tr>
					...
					<th class="w150">{t domain="comment"}评级{/t}</th>
				</tr>
			</thead>
			<tbody>
					...
					<td class="hide-edit-area">
						...
						<div class="edit-list">
						    {if $comment.status lt 2}
								<a class="toggle_view" href='{url path="comment/admin/check" args="list=3&comment_id={$comment.comment_id}&status={$comment.status}{if $smarty.get.page}&page={$smarty.get.page}{/if}{if $store_id}&store_id={$store_id}{/if}"}' data-msg='{t domain="comment"}您确定要更改此评论的状态吗？{/t}' data-val="{if $comment.status eq 0}allow{else}forbid{/if}" data-status="{$smarty.get.status}" >
				...
				<tr style="border-top:none;">
					<td colspan="5" style="border-top:none;">
						<div style="border-top: 2px dashed #ddd;">
							<input class="form-control small span12" style="width:94%;margin-bottom:3px;margin-top:12px;" value="" name="reply_content" type="text" placeholder='{t domain="comment"}感谢您对本店的支持！我们会更加的努力，为您提供更优质的服务。（可在此输入回复内容，也可选择系统自动回复）{/t}'>
							<input class="btn btn-gebo quick_reply" style="height:36px;margin-top:9px;" type="button" data-url="{url path='comment/admin/quick_reply'}" data-id={$comment.comment_id} data-status={$comment.status} value='{t domain="comment"}回复{/t}' data-list={3} />
						</div>
					</td>
				</tr>
				<!-- {foreachelse} -->
